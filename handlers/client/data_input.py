...
async def input_uc(msg: types.Message, state: FSMContext):
  ...
  await msg.answer("PUBG Mobile global id kirgaing ")
...
async def input_cancel(msg: types.Message, state: FSMContext):
  await msg.answer("Bekor boldi")
  await state.finish()

@dp.message_handler(state=Input_Uc_count.photo, content_types=['text'])
...
async def input_photo(msg: types.Message, state: FSMContext):
  ...
  await msg.answer("Qorib chikiladi")
